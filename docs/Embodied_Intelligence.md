...
- **Agent**: An entity that can perform actions and interact within the MAS environment.
- **Prompt**: A directive or instruction given to an agent to perform a specific task.
- **Response**: The outcome or output generated by an agent after processing a prompt.

### Using `add_controller` Agent in MAS

In this section, we will demonstrate how to use the `add_controller` agent within the MAS framework. Specifically, we will illustrate the process of initializing the agent, loading a prompt, and generating a response.

#### Step 1: Initialize the Agent

First, initialize the agent responsible for adding controllers.

```python
from LLM.agent import AgentLLM as Agent

# Initialize the add_controllers agent
agent_add_controllers = Agent("add_controllers", save_path=LOG_PATH)
```

#### Step 2: Load the Prompt
Load the system prompt from a file. This prompt defines the task for the agent.

```python
# Load the system prompt for the add_controllers agent
agent_add_controllers.load_system_prompt_from_file(PROMPTS_PATH + '/add_controller_prompt.txt')
```

#### Step 3: Generate a Response
Define a user prompt that describes the controllers to be added and generate a response from the agent.

```python
# Generate the response from the agent
response = agent_add_controllers.generate_response(controllers_str)
print("Agent's Response:", response)
```

### Example Workflow for Adding Controllers
Below is an example workflow that demonstrates the entire process of adding controllers using the add_controllers agent.
...
controllers_str = """
Add a controller for the robotic arm to handle picking and placing objects.
"""
